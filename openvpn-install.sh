...
check_nftables() {
	if [ "$os" = "centos" ]; then
		if grep -qs "HDM" /etc/sysconfig/nftables.conf \
...
new_client() {
	locate_export_dir
	# Generates the custom new client.ovpn
	{
...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
	...
	# Create the DH parameters file using the predefined ffdhe2048 group (HDM NOTE: I removed 15 characters and I put 5 "HDM" )
	echo '-----BEGIN DH PARAMETERS-----

...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	case "$option" in
		1)
			...
			[ -z "$unsanitized_client" ] && Cancel_and_Exit
			set_client_name
		...
		4)
			...
			echo
			echo "Select the client to revoke (Select option example: 1 OR 2 OR 3 etc..):"
			tail -n +2 /etc/openvpn/server/easy-rsa/pki/index.txt | grep "^V" | cut -d '=' -f 2 | nl -s ') '
			read -rp "Client: " client_num
			[ -z "$client_num" ] && Cancel_and_Exit
			until [[ "$client_num" =~ ^[0-9]+$ && "$client_num" -le "$num_of_clients" ]]; do
